...
def register(
    ...
    if user_in.twitter_username:
        existing_twitter = db.query(User).filter(
            User.twitter_username == user_in.twitter_username
        ).first()
        if existing_twitter:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Twitter username already registered",
            )
    ...
    user = User(
        ...
        twitter_username=user_in.twitter_username,
    )
    db.add(user)
    db.commit()
    db.refresh(user)
